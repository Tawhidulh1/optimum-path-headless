...
/**
 * AStar class implements the A* pathfinding algorithm to find the shortest path
 * in a graph.
 * The graph is represented using an adjacency list, and the algorithm uses a
...
public final class AStar {
  ...
  /**
   * Implements the A* pathfinding algorithm to find the shortest path from a
   * start node to a destination node.
   *
   * @param from      the starting node
   * @param to        the destination node
   * @param graph     the graph representation of the problem
   * @param heuristic the heuristic estimates for each node
   * @return a PathAndDistance object containing the shortest path and its
   *         distance
   */
  public static PathAndDistance aStar(int from, int to, Graph graph, int[] heuristic) {
    // PriorityQueue to explore nodes based on their distance and estimated cost to
      ...
      if (currentPosition == to) {
        ...
        for (Edge edge : graph.getNeighbours(currentPosition)) {
          ...
          if (!currentData.getPath().contains(edge.getTo())) {
            ...
            queue.add(new PathAndDistance(currentData.getDistance() + edge.getWeight(), updatedPath,
